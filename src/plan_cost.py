...
def compute_plan_totals(rows: list[dict]) -> dict:
    ...
    return {
        "start": "TOTAL",
        "production": round(sum(float(r.get("production") or 0) for r in rows), 2),
        "consumption": round(sum(float(r.get("consumption") or 0) for r in rows), 2),
        "battery": round(sum(float(r.get("battery") or 0) for r in rows), 2),
        ...
        "grid_import": round(sum(float(r.get("grid_import") or 0) for r in rows), 2),
        "grid_export": round(sum(float(r.get("grid_export") or 0) for r in rows), 2),
        "soc": float(rows[-1].get("soc") or 0),
        "import_cost": round(import_cost, 4),
        "export_revenue": round(export_revenue, 4),
